...
<div id="content" class="container col-md-12">
	<?php
		...
		if(isset($_POST['btnSave'])){
			...
			// check update result
			if($update_result){
				$error['update_data'] = " <span class='label label-primary'>주문 정보 변경 완료</span>";
			}else{
				$error['update_data'] = " <span class='label label-danger'>주문 정보 변경 실패</span>";
			}
		}
...
<div class="col-md-7 col-md-offset-2">
	...
	<br>
		<table table class='table table-bordered table-condensed'>
			...
			<tr class="row">
				<th class="detail active">상태</th>
				<td class="detail">
					<select name="status" class="form-control">
						<?php if($data['Status'] == 1){ ?>
							<option value="1" selected="selected">처리</option>
							<option value="0" >미처리</option>
						<?php }else{?>
							<option value="1" >처리</option>
							<option value="0" selected="selected">미처리</option>
						<?php }?>
					</select>
				</td>
			</tr>
