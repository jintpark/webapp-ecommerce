...
<div id="content" class="container col-md-12">
		...
		if(isset($_POST['btnEdit'])){
			...
			if(!empty($category_name) && empty($error['category_image'])){
				...
				if($update_result){
					...
					$error['update_category'] = " <span class='label label-danger'>카테고리 편집 실패</span>";
				}
			}
	...
	<div class="col-md-5">
		<form method="post"
			...
			<label>카테고리명 :</label><?php echo isset($error['category_name']) ? $error['category_name'] : '';?>
			<input type="text" class="form-control" name="category_name" value="<?php echo $data['Category_name']; ?>"/>
			<br/>
			<label>대표이미지 :</label><?php echo isset($error['category_image']) ? $error['category_image'] : '';?>
			<input type="file" name="category_image" id="category_image" /><br />
			<img src="<?php echo $data['Category_image']; ?>" width="280" height="190"/>
			<br/><br/>
			<input type="submit" class="btn-primary btn" value="업데이트" name="btnEdit"/>
			<input type="submit" class="btn-danger btn" value="취소" name="btnCancel"/>
		</form>
	</div>
