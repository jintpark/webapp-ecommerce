...
<div id="content" class="container col-md-12">
	<?php
		...
		if(isset($_POST['btnEdit'])){
			...
			if(empty($menu_name)){
				$error['menu_name'] = " <span class='label label-danger'>메뉴명을 입력하세요!</span>";
			}

			if(empty($category_ID)){
				$error['category_ID'] = " <span class='label label-danger'>카테고리를 선택하세요!</span>";
			}

			if(empty($price)){
				$error['price'] = " <span class='label label-danger'>가격을 정해주세요!</span>";
			}else if(!is_numeric($price)){
				$error['price'] = " <span class='label label-danger'>가격은 숫자만 입력가능합니다!</span>";
			}

			if(empty($quantity)){
				$error['quantity'] = " <span class='label label-danger'>수량을 입력하세요!</span>";
			}else if(!is_numeric($quantity)){
				$error['quantity'] = " <span class='label label-danger'>수량은 숫자만 입력가능합니다!</span>";
			}
			...
			if(empty($description)){
				$error['description'] = " <span class='label label-danger'>설명을 입력하세요!</span>";
			}
			...
			if(!empty($menu_image)){
				if(!(($image_type == "image/gif") ||
					...
					$error['menu_image'] = "*<span class='label label-danger'>이미지는 jpg, jpeg, gif, png 형태로 올리셔야 합니다!</span>";
				}
			}
			...
			if(!empty($menu_name) && !empty($category_ID) && !empty($price) && is_numeric($price) &&
				...
				// check update result
				if($update_result){
					$error['update_data'] = " <span class='label label-primary'>편집성공</span>";
				}else{
					$error['update_data'] = " <span class='label label-danger'>편집실패</span>";
				}
			}
	...
	?>
	<div class="col-md-12">
	<h1>메뉴 편집 <?php echo isset($error['update_data']) ? $error['update_data'] : '';?></h1>
	<hr />
	</div>
	...
	<div class="col-md-9">
		<div class="col-md-12">
			<label>메뉴명 :</label><?php echo isset($error['menu_name']) ? $error['menu_name'] : '';?>
			<input type="text" name="menu_name" class="form-control" value="<?php echo $data['Menu_name']; ?>"/>
		</div>

		<div class="col-md-3">
	    <br>
	    <label>가격 (<?php echo $currency;?>) :</label><?php echo isset($error['price']) ? $error['price'] : '';?>
		<input type="text" name="price" class="form-control" value="<?php echo $data['Price'];?>"/>
		<br/>

		<label>수량 :</label><?php echo isset($error['quantity']) ? $error['quantity'] : '';?>
		<input type="text" name="quantity" class="form-control" value="<?php echo $data['Quantity'];?>"/>
		<br/>

	    <label>상태 :</label><?php echo isset($error['serve_for']) ? $error['serve_for'] : '';?>
		<select name="serve_for" class="form-control">
			<option>구매가능</option>
			<option>매진</option>
		</select>
		<br/>
	    <label>카테고리 :</label><?php echo isset($error['category_ID']) ? $error['category_ID'] : '';?>
	    ...
	    <br/>
		<label>대표이미지 :</label><?php echo isset($error['menu_image']) ? $error['menu_image'] : '';?>
		...
		<div class="col-md-9">
		<br>
		<label>설명 :</label><?php echo isset($error['description']) ? $error['description'] : '';?>
	...
	<br/>
		<div class="panel panel-default">
				...
				<div class="panel-body">
					<input type="submit" class="btn-primary btn" value="수정" name="btnEdit" />
				</div>
		</div>
	</div>
	</form>
